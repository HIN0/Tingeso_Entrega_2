...
function ToolList() {
  ...
  // --- NUEVA LÓGICA DE AUTENTICACIÓN (Sin Keycloak) ---
  ...
  if (isAuth) {
    try {
        // Decodificamos el payload del JWT (la parte del medio) para leer el rol
        const payload = JSON.parse(atob(token.split('.')[1]));
        // M7 Auth Service guarda el rol bajo la llave "role"
        isAdmin = payload.role === "ADMIN";
    } catch (e) {
        console.error("Error al leer el rol del token:", e);
    }
  }
  // ----------------------------------------------------
                  ...
                  {adjustment.id === tool.id ? (
                      ...
                      <>
                        <button
                          ...
                          style={{ marginRight: '5px', backgroundColor: 'black', color: 'white' }}
